...
public class ResourceManager extends CompositeService implements Recoverable {
  ...
  public static boolean use_coflow;
  public List<UpdateDscpThread> UpdateDscpThreadList = null;
  public ListenNMConnectionThread listenNMConnectionThread = null;
  public NodeReducerInfoThread nodeReducerInfoThread = null;
  ...
  public void UpdateDscp(String message){
    if(message.length() == 0) return;
    LOG.info("Update Dscp : " + message);
    for(UpdateDscpThread udt : UpdateDscpThreadList){
      synchronized (udt){
        udt.updateDscp(message);
        udt.notify();
      }
    }
  }

  protected class ListenNMConnectionThread extends Thread{
    private ServerSocket serverSocket;
    private Socket clientSocket;
    public void run(){
      try{
        InetSocketAddress NMConnectionAddress = new InetSocketAddress("172.16.100.1", 60003);
        serverSocket = new ServerSocket();
        serverSocket.bind(NMConnectionAddress);
        LOG.info("Begin to listen NM connection.....");
        while (true){
          clientSocket = serverSocket.accept();
          UpdateDscpThread udt  = new UpdateDscpThread(clientSocket);
          UpdateDscpThreadList.add(udt);
          udt.start();
        }
      }catch (IOException e){
        e.printStackTrace();
  ...
  protected class UpdateDscpThread extends Thread{
    Socket socket;
    boolean running;
    String message;
    public UpdateDscpThread(Socket socket){
      this.socket = socket;
      this.message = "";
    }
      ...
      try{
        ...
        synchronized (this){
          do{
            running = false;
            this.wait();
            //Here is some code use to begin to send dscp value to NM
            if(message.length() == 0){
              running = true;
            }else{
              pw.write(message + "\n");
  ...
  protected class NodeReducerInfoThread extends Thread{
    private ServerSocket serverSocket;
    private Socket clientSocket;
    public void run(){
      try{
        InetSocketAddress nodeReducerInfoAddress = new InetSocketAddress("172.16.100.1", 60001);
        serverSocket = new ServerSocket();
        serverSocket.bind(nodeReducerInfoAddress);
        LOG.info("The node reducer info server starts.");
        while(true){
          clientSocket = serverSocket.accept();
          new NodeReducerInfoDealThread(clientSocket).start();
        }
      }catch (IOException e){
        e.printStackTrace();
      }finally {
  ...
  protected class NodeReducerInfoDealThread extends Thread{
    private Socket socket;
    private BufferedReader br;
    public NodeReducerInfoDealThread(Socket socket){
      this.socket = socket;
    }
    public void run(){//received reducer info from ReduceTask
