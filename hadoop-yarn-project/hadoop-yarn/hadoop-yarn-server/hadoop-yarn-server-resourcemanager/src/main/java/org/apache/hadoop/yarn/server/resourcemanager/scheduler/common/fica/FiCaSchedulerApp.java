  ...
  public RMContainer allocate(FiCaSchedulerNode node,
    ...
    try {
      ...
      if (getOutstandingAsksCount(schedulerKey) <= 0) {
        LOG.info("getOutstandingAsksCount(schedulerKey) <= 0)");
        return null;
      }
    ...
    try {
      ...
      if (request.anythingAllocatedOrReserved()) {
        ...
        if (schedulerContainer.isAllocated()) {
          ...
          if(ResourceManager.use_coflow){

          }

          LOG.info("The schedulerContainer is: " + schedulerContainer + ".");
