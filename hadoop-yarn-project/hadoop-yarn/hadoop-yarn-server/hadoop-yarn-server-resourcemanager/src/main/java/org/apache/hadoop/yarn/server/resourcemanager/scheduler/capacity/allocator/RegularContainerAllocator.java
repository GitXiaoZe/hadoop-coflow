...
public class RegularContainerAllocator extends AbstractContainerAllocator {
  ...
  ContainerAllocation tryAllocateOnNode(Resource clusterResource,
    ...
    // LOG.info("tryAllocateOnNode: " + result.getAllocationState() + ", and this time, the schedulerKey: " + application.getOutstandingAsksCount(schedulerKey));
  ...
  private int getActualNodeLocalityDelay() {
    ...
    //return application.getCSLeafQueue().getNodeLocalityDelay();
  }
  ...
  private boolean canAssign(SchedulerRequestKey schedulerKey,
    ...
    if (type == NodeType.OFF_SWITCH) {
      ...
      /*
      if(ResourceManager.use_max_reducer) {
        Priority PRIORITY_REDUCE = RecordFactoryProvider.getRecordFactory(null).newRecordInstance(Priority.class);
        PRIORITY_REDUCE.setPriority(10);
        if(schedulerKey.getPriority().equals(PRIORITY_REDUCE)) {
          // LOG.info("runing our code: normal logical");
          ResourceManager.NodeShuffleInfo nodeShuffleInfo = ResourceManager.nodeShuffleInfos.get(node.getNodeName());
          if (nodeShuffleInfo.shuffleReduceNum >= ResourceManager.max_reducer_num) {
            // LOG.info("REDUCE_NUM_SKIPPED");
            // // LOG.info("the node(" + node.getNodeName() + ")\'s shuffle reduce num is great than max_reducer_num(" + ResourceManager.max_reducer_num + ")!");
            // ActivitiesLogger.APP.recordSkippedAppActivityWithoutAllocation(
            //   activitiesManager, node, application, priority,
            //   ActivityDiagnosticConstant.REDUCE_NUM_SKIPPED);
            // return ContainerAllocation.REDUCE_NUM_SKIPPED;
            return false;
          }
          // long minShuffleSize = Long.MAX_VALUE;
          // for (ResourceManager.NodeShuffleInfo nsi : ResourceManager.nodeShuffleInfos.values()) {
          //   if(nsi.shuffleSize < minShuffleSize) {
          //     minShuffleSize = nsi.shuffleSize;
          //   }
          // }
          // if (nodeShuffleInfo.shuffleSize != minShuffleSize) {
          //   LOG.info("SHUFFLE_SIZE_SKIPPED");
          //   // LOG.info("the node(" + node.getNodeName() + ")\'s shuffle reduce size is not minimum!");
          //   // ActivitiesLogger.APP.recordSkippedAppActivityWithoutAllocation(
          //   //   activitiesManager, node, application, priority,
          //   //   ActivityDiagnosticConstant.SHUFFLE_SIZE_SKIPPED);
          //   // return ContainerAllocation.SHUFFLE_SIZE_SKIPPED;
          //   return false;
          // }
        }
      }
  ...
  private ContainerAllocation assignContainersOnNode(Resource clusterResource,
    ...
    // Data-local
    // LOG.info("assignContainersOnNode, data local, this time the scheudlerKey is: " + application.getOutstandingAsksCount(schedulerKey));
    ...
    // Rack-local
    // LOG.info("assignContainersOnNode, rack local, this time the scheudlerKey is: " + application.getOutstandingAsksCount(schedulerKey));
    ...
    // Off-switch
    // LOG.info("assignContainersOnNode, off switch, this time the scheudlerKey is: " + application.getOutstandingAsksCount(schedulerKey));
  ...
  private ContainerAllocation assignContainer(Resource clusterResource,
    ...
    // LOG.info("assignContainers condition: node=" + node.getNodeName()
    //       + " application=" + application.getApplicationId()
    //       + " priority=" + schedulerKey.getPriority()
    //       + " pendingAsk=" + pendingAsk + " type=" + type);
    ...
    if (availableContainers > 0) {
      ...
      // LOG.info("container allocated. at this time the scheudlerKey is: " + application.getOutstandingAsksCount(schedulerKey));
      ...
      // LOG.info("availableContainers <= 0");
  ...
  private ContainerAllocation handleNewContainerAllocation(
    ...
    // LOG.info("the allocationResult.updatedContainer is " + allocatedContainer);
  ...
  ContainerAllocation doAllocation(ContainerAllocation allocationResult,
    ...
    if (allocationResult.getAllocationState() == AllocationState.ALLOCATED) {
      ...
      // LOG.info("doAllocation: " + allocationResult.getAllocationState());
    } else {
  ...
  private ContainerAllocation allocate(Resource clusterResource,
    ...
    if (reservedContainer == null) {
      ...
      if (null != result) {
        LOG.info("condition: pre return. and the result's allocation state is " + result.getAllocationState());
        return result;
      }
    } else {
    ...
    while (iter.hasNext()) {
      ...
      // LOG.info("this node is " + node.getNodeName());

      // LOG.info("before tryAllocateOnNode, the schedulerKey: " + application.getOutstandingAsksCount(schedulerKey));
      result = tryAllocateOnNode(clusterResource, node, schedulingMode,
          resourceLimits, schedulerKey, reservedContainer);
      // LOG.info("after tryAllocateOnNode, the schedulerKey: " + application.getOutstandingAsksCount(schedulerKey));
    ...
    // LOG.info("after do allocation, the result is " + result.getAllocationState() + ", the schedulerKey: " + application.getOutstandingAsksCount(schedulerKey));
  ...
  public CSAssignment assignContainers(Resource clusterResource,
    ...
    if (reservedContainer == null) {
      ...
      if (!application.hasPendingResourceRequest(rc,
        ...
        // LOG.info("APPLICATION_DO_NOT_NEED_RESOURCE");
      ...
      // LOG.info("SKIPPED_ALL_PRIORITIES");
      return CSAssignment.SKIP_ASSIGNMENT;
    } else {
      // LOG.info("condition: allocate reserved container.");
      ContainerAllocation result =
