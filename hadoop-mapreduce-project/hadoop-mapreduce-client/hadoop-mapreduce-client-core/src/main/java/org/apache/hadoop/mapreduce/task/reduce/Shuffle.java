...
public class Shuffle<K, V> implements ShuffleConsumerPlugin<K, V>, ExceptionReporter {
  ...
  public void init(ShuffleConsumerPlugin.Context context) {
    ...
    this.use_coflow = jobConf.getBoolean(YarnConfiguration.USE_COFLOW, YarnConfiguration.DEFAULT_USE_COFLOW);
  }
  ...
  protected class WaitForFetcherFinishThread extends Thread{
    String job_id;
    int reducer_id;
    long FetcherTag;
    public WaitForFetcherFinishThread(int FetcherNum, String job_id, int reducer_id){
      this.job_id = job_id;
      this.reducer_id = reducer_id;
      FetcherTag = (1 << FetcherNum) - 1;
    }
    public void run(){
      LOG.info("WaitForFetcherFinishThread start......");
      synchronized (this){
        try{
          while(FetcherTag != 0)
            this.wait();
      ...
      try{
        Socket socket = new Socket("172.16.100.1", 60001);
        ...
        pw.close();
        socket.close();
        LOG.info("WaitForFetcherFinishThread finish......");
      }catch(IOException e){
        e.printStackTrace();
      }
    }

    public void UpFetcherFlag(int Fetcher_id){
      FetcherTag &= (~(1 << Fetcher_id));
      LOG.info("FetcherTag-"+FetcherTag);
    }
  }
}
