...
public class SpillRecord {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(SpillRecord.class.getName());
  ...
  public void putIndex(IndexRecord rec, int partition) {
	...
	// LOG.info("pos: " + pos + ", " + "startOffset: " + rec.startOffset);
	// LOG.info("pos: " + (pos + 1) + ", " + "rawLength: " + rec.rawLength);
	// LOG.info("pos: " + (pos + 2) + ", " + "partLength: " + rec.partLength);
  }
  ...
  public void writeToFile(Path loc, JobConf job, Checksum crc)
    ...
    try {
      if (crc != null) {
        ...
        // for (int i = 1; i < entries.array().length; i += 3) {
        //   int temp = i/3 + 1;
        //   LOG.info("The " + temp + "reducer's intermediate outputs: " + entries.get(i));
        // }
        LOG.info("test the Path toString() function");
        LOG.info(loc.getParent().getName());
        LOG.info(loc.getParent().toString());
	  	  // LOG.info("checksum: " + chk.getChecksum().getValue());
      } else {
        out.write(buf.array());
      }
    } finally {
  ...
  public void writeIntermediateOutput(String path) throws IOException {
    ...
    try {
      for (int i = 0; i < size(); i ++) {
        // int temp = i/3 + 1;
        LOG.info("The " + i + " reducer's intermediate outputs: " + entries.get(3*i + 1));
        dos.writeLong(entries.get(3*i + 1));
      }
    } finally {
      dos.close();
    }
  ...
  public void writeToNodeManager(String job_id){
    String ret = "";
    int i;
    int length = size();
    ...
    ret += entries.get(3*i+1);
    LOG.info("MapTask begin to write NM : " + ret);
    //send to NM;
    Socket socket;
    BufferedWriter bw;
    try{
