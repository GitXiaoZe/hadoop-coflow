...
public class RMContainerAllocator extends RMContainerRequestor
  ...
  */
  ...
  protected void serviceInit(Configuration conf) throws Exception {
    ...
    LOG.info("use_coflow = " + use_coflow);
    ...
    if (!getIsReduceStarted()) {//not set yet
      ...
      if(completedMaps < completedMapsForReduceSlowstart) {
        ...
        if(use_coflow){
          ...
          if(MRAppMaster.getMapIntermediateOutputThread != null){
            MRAppMaster.getMapIntermediateOutputThread.start();
          }
          ...
          try{
            socket_to_RM = new Socket("172.16.100.1", 60000);
            oos = new ObjectOutputStream(socket_to_RM.getOutputStream());
            oos.writeObject(MRAppMaster.reduceInfos);
            oos.flush();
            LOG.info("socket connected with 172.16.100.1:60000, and transfer reduceInfos : " + MRAppMaster.reduceInfos);
          }catch(IOException e){
            throw new YarnRuntimeException(e);
          }finally {
            try{
