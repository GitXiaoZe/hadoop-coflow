...
public class MRAppMaster extends CompositeService {
  ...
  public static void main(String[] args) {
    try {
      ...
      getMapIntermediateOutputThread = new GetMapIntermediateOutputThread(applicationAttemptId, jobUserName);
  ...
  // Start a new thread to get the map task intermedia output
  static public class GetMapIntermediateOutputThread extends Thread {
    ApplicationAttemptId applicationAttemptId;
    String jobUserName;
    GetMapIntermediateOutputThread(ApplicationAttemptId applicationAttemptId, String jobUserName) {
    ...
    public void run() {
      ...
      String tempIntermediateOutputDir = hadoopTmpDir + applicationAttemptId.getApplicationId().toString();
      File file = new File(tempIntermediateOutputDir);
      if(!file.exists()) {
        file.mkdir();
      }
      Process fetchProcess = null;
      try {
        Runtime runtime = Runtime.getRuntime();
        for (int i = 2; i <= 10; i ++){
          String bashCommand = "scp tian@172.16.100." + i + ":" + intermediateOutputDir + "/*_output " + tempIntermediateOutputDir;
          // LOG.info("execute command: " + bashCommand);
          fetchProcess = runtime.exec(bashCommand);
          int status = fetchProcess.waitFor();
          if(status != 0){
            LOG.info("Failed to call shell's command.");
            BufferedReader br = new BufferedReader(new InputStreamReader(fetchProcess.getErrorStream()));
            String s;
            while( (s = br.readLine()) != null )
              LOG.info("Error message : " + s);
          }
          // LOG.info("the fetch Process exit value: " + fetchProcess.exitValue());
        }
      }
      catch(InterruptedException e) {
        e.printStackTrace();
        LOG.info("there is a InterruptedException.");
      }
      catch(IOException e){
        e.printStackTrace();
        LOG.info("there is a IOException.");
      }
      
      File[] intermediateOutputList = file.listFiles();
      DataInputStream dis = null;
      LOG.info("the dir have " + intermediateOutputList.length + " files.");
      for(File thisFile: intermediateOutputList) {
        try {
          String host_name = thisFile.getName();
          int last_flag = host_name.lastIndexOf('_');
          int next_to_last_flag = host_name.lastIndexOf('_', last_flag - 1);
          host_name = host_name.substring(next_to_last_flag + 1, last_flag);
          String host_id = host_name.substring(4);
          int reduce_index = 0;
          dis = new DataInputStream(new FileInputStream(thisFile));
          while(true) {
            long tLong = dis.readLong();
            //LOG.info("the file " + thisFile + " has a long data: " + tLong);
            //reduceInfos[index].reduceSize += tLong;
            reduceInfos.put(Integer.valueOf(host_id), reduce_index, tLong);
            reduce_index ++;
          }
        }
        catch(FileNotFoundException e) {
          LOG.info(thisFile + " is not exists.");
        }
        catch(EOFException e) {
          // LOG.info("file is ending. this file has " + index + " long datas. ");
        }
        catch(IOException e) {
          LOG.info("there is a IOException. ");
        }
